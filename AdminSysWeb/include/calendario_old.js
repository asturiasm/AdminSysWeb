...
// 	Modificado por Ricaute Jimenez S.
...
function PoPCalCreateCalendarInstance()
{
	...
	if (!PopCalInstanceCreated())
	{
		...
		this.showWeekNumber = 0				// 0 - dont show, 1 - show
		this.showToday = 1					// 0 - dont show, 1 - show
		this.showWeekend = 0				// 0 - dont show, 1 - show
		this.showHolidays = 1				// 0 - dont show, 1 - show
		this.showSpecialDay = 1				// 0 - dont show, 1 - show
		this.selectWeekend = 1				// 0 - dont Select, 1 - Select
		this.selectHoliday = 1				// 0 - dont Select, 1 - Select
		this.addCarnival = 0				// 0 - dont Add, Add to Holiday (Tuesday)
		this.addGoodFriday = 0				// 0 - dont Add, Add to Holiday
		...
		this.fade = 0						// 0 - dont fade, .1 to 1 - fade (Only IE) 
		this.shadow = 0						// 0  - dont shadow, 1 - shadow
		this.move = 0						// 0  - dont move, 1 - move
		this.saveMovePos = 0				// 0  - dont save, 1 - save
		...
		this.imgDir = "../img/calendario/"  // directory for images ... e.g. var imgDir="/img/"
		
		this.gotoString = ""
		this.todayString = "Dia de hoy"
		this.weekString = ""
		this.scrollLeftMessage = "Mes anterior"
		this.scrollRightMessage = "Mes siguiente"
...
function PopCalInitCalendar()
{
	...
	if (PopCal)
	{
		if (PopCal.initialized==0)
		{
			...
			document.write ("<div id='popupSuperCalendar' onclick='PopCalDownMonth();PopCalDownYear();objPopCalendar.bShow=true;' style='z-index:+999;position:absolute;top:0;left:0;visibility:hidden;background-color:#ffffff'><table id='popupSuperHighLight' width="+((objPopCalendar.showWeekNumber==1)?350:220)+" style='font-family:verdana;font-size:11px;border-width:1;border-style:solid;border-color:#a0a0a0;font-family:arial; font-size:11px}' bgcolor='#ffffff'><tr bgcolor='#0000aa'><td Style='cursor:default'><table width='"+((objPopCalendar.showWeekNumber==1)?248:218)+"'><tr><td style='padding:2px;font-family:arial; font-size:11px;cursor:default'><font color='#ffffff'><B><span id='popupSuperCaption'></span></B></font></td><td id='popupSuperMoveCalendar' width='1px'></td><td align=right Style='cursor:default'><a onClick='ImgCloseBoton.src=\""+ objPopCalendar.imgDir + "close.gif\";objPopCalendar.PopCalTimeOut=window.setTimeout(\"window.clearTimeout(objPopCalendar.PopCalTimeOut);objPopCalendar.PopCalTimeOut=null;PopCalHideCalendar()\",100)'><IMG  title='Cerrar calendario' ID=ImgCloseBoton SRC='"+objPopCalendar.imgDir+"close.gif' onMouseOver='if(objPopCalendar.PopCalDragClose){this.src=\""+ objPopCalendar.imgDir + "closedown.gif\"}' onMouseDown='this.src=\""+ objPopCalendar.imgDir + "closedown.gif\"' onMouseUp='this.src=\""+ objPopCalendar.imgDir + "close.gif\"' onMouseOut='this.src=\""+ objPopCalendar.imgDir + "close.gif\"' onDrag='objPopCalendar.PopCalDragClose=true;return(false)' WIDTH='15' HEIGHT='13' BORDER='0'></a></td></tr></table></td></tr><tr><td style='padding:5px;cursor:default' bgcolor=#ffffff><span id='popupSuperContent'></span></td></tr>"+"</a>")
			document.write ("<td colspan='5' align='center'><a title='Borrar fecha elegida y cierra calendario' href='javascript:limpiarfecha();'>[limpiar fecha]</a></td>")
			...
			if (objPopCalendar.language == 0)
			{
				...
				{
					objPopCalendar.dayName = new Array ("Domingo","Lunes","Martes",mi,"Jueves","Viernes","Sabado")
				}
				else
				{
					objPopCalendar.dayName = new Array ("Lunes","Martes",mi,"Jueves","Viernes","Sabado","Domingo")
				}
			...
			if (objPopCalendar.language == 0)
			{
				objPopCalendar.todayString = "Hoy es " + objPopCalendar.dayName[(objPopCalendar.dayNow-objPopCalendar.startAt==-1)?6:(objPopCalendar.dayNow-objPopCalendar.startAt)]+ ", " + objPopCalendar.dateNow + " de " + objPopCalendar.monthName[objPopCalendar.monthNow] + " " + objPopCalendar.yearNow
			}
			else
			{
			...
			if (objPopCalendar.showToday==1)
			{
				document.getElementById("popupSuperToday").innerHTML = "<a onmouseover='window.status=\""+objPopCalendar.gotoString+"\"' onmouseout='window.status=\"\"' title='"+objPopCalendar.gotoString+"' style='"+objPopCalendar.styleAnchor+"' onClick='PopCalChangeCurrentMonth();'>" + objPopCalendar.todayString
			}
...
function PopCalMoveDefaultPos()
{
	...
	if (((leftpos + oCal.clientWidth + 10) - aTag.scrollLeft) > aTag.clientWidth)
	{
		leftpos -= ((((leftpos + oCal.clientWidth) - aTag.clientWidth) + 10) - aTag.scrollLeft)
	}
	...
	if (((toppos + oCal.clientHeight + 65) - aTag.scrollTop) > aTag.clientHeight)
	{
		toppos -= ((((toppos + oCal.clientHeight) - aTag.clientHeight) + 65) - aTag.scrollTop)
	}
...
function PopCalConstructCalendar ()
{
	...
	{
		sHTML += "<td width=27 Style='cursor:default'><b>"+ objPopCalendar.weekString + "</b></td><td width=1 rowspan=8 bgcolor='#d0d0d0' style='padding:0px'></td>"
	}

	for (var i=0; i<7; i++)
	{
		sHTML += "<td width='27' align='right' Style='cursor:default'><B>"+ objPopCalendar.dayName[i].substr(0,2)+"</B></td>"
	}
	sHTML +="</tr><tr>"
	...
	//  Martes de Carnaval y Viernes Santo para el ano actual
	...
	document.getElementById("popupSuperSpanMonth").innerHTML = "&nbsp;" + objPopCalendar.monthName[objPopCalendar.monthSelected] + "&nbsp;<IMG id='popupSuperChangeMonth' onDrag='return(false)' SRC='"+objPopCalendar.imgDir+"drop1.gif' WIDTH='12' HEIGHT='10' BORDER=0>"
	document.getElementById("popupSuperSpanYear").innerHTML = "&nbsp;" + objPopCalendar.yearSelected + "&nbsp;<IMG id='popupSuperChangeYear' onDrag='return(false)' SRC='"+objPopCalendar.imgDir+"drop1.gif' WIDTH='12' HEIGHT='10' BORDER=0>"
...
{
	//  aqui se agregan los dias libres del ano para que el calendario 
	//  los despliege si esta opcion esta activada
	PopCalAddHoliday(1,1,0,"A" + c_nTilde + "o Nuevo","Happy New Year")
	PopCalAddHoliday(15,9,0,"D" + c_iTilde + "a de la Independencia", "Independence Day")
	PopCalAddHoliday(1,11,0,"D" + c_iTilde + "a de los Santos", "Saints day")
	PopCalAddHoliday(24,12,0,"Navidad", "Christmas")
}


function iniCal()
{
	//mi='Mi' + c_eTilde + 'rcoles'
	mi="Mi&eacute;rcoles"
	calendario = getCalendarInstance()	// instancia del calendario 
	calendario.startAt = 0				// 0 - sunday  1 - monday
	calendario.showWeekNumber = 0		// 0 - dont show 1 - show
	calendario.showToday = 1			// 0 - dont show 1 - show
	calendario.showWeekend = 0			// 0 - dont show 1 - show
	calendario.showHolidays = 0			// 0 - dont show 1 - show
	calendario.showSpecialDay = 0		// 0 - dont show 1 - show
	calendario.selectWeekend = 1		// 0 - dont Select 1 - Select
	calendario.selectHoliday = 0		// 0 - dont Select 1 - Select
	calendario.addCarnival = 0			// 0 - dont Add Add to Holiday (Tuesday of Carnival)
	calendario.addGoodFriday = 1		// 0 - dont Add Add to Holiday
	calendario.language = 0				// 0 - Spanish 1 - English
	calendario.defaultFormat = "dd/mm/yyyy" //  Default Format dd-mm-yyyy
	calendario.fixedX = -1				// x position (-1 if to appear below control)
	calendario.fixedY = -1				// y position (-1 if to appear below control)
	calendario.fade = 0					// 0 - dont fade 1 to 1 - fade (Only IE) 
	calendario.shadow = 0				// 0  - dont shadow 1 - shadow
	calendario.move = 0					// 0  - dont move 1 - move (Only IE)
	calendario.saveMovePos = 1			// 0  - dont save 1 - save
	calendario.centuryLimit = 40		// 1940 - 2039
	//  calendario.forcedToday("31-12-1999", "dd-mm-yyyy")  //   Force Today Date
...
// --------------------------------------------------------------------------
// ********************funciones para uso del calendario*********************
// --------------------------------------------------------------------------
function okdate(control)
{
	if (control.value != control.oldValue)
	{
		var sDate = calendario.formatDate(control.value, "dd.mmm.yyyy", "dd.mmm.yyyy")
		if (sDate != "")
		{
			var re = /(\w+)-(\w+)-(\w+)/
			alert("Usted elgio el " + sDate.replace(re,"$1 de $2 de $3") + '.')
		}
	}
	window.Form1.click();
}
...
function checkDate(o, f)
{
	...
	var sDate = calendario.formatDate(o.value, f, "yyyy-mm-dd")

	if ((sDate == "") || (sDate < "1900-01-01"))
	{
		o.value = ""
	}
}
...
function VerifyDate()
{
	...
	var vDesde = calendario.formatDate(document.Prueba.DesdeFecha.value, "mmm.dd.yyyy", "yyyy-mm-dd")
	if (vDesde=="") 
	{
		vDesde = "1900-01-01"
	}
	var vHasta = calendario.formatDate(document.Prueba.HastaFecha.value, "yyyy/mmm/dd", "yyyy-mm-dd")
	if (vHasta=="") 
	{
		vHasta = "2099-12-31"
	}
	var vRango = calendario.formatDate(document.Prueba.RangoFecha.value, "mmmm-dd-yyyy", "yyyy-mm-dd")
	var vTemp

	if (vDesde > vHasta)
	{
		document.Prueba.HastaFecha.value = calendario.formatDate(vDesde, "yyyy-mm-dd", "yyyy/mmm/dd")
		document.Prueba.DesdeFecha.value = calendario.formatDate(vHasta, "yyyy-mm-dd", "mmm.dd.yyyy")
	...
	if (vRango != "")
	{
		if ((vRango < vDesde) || (vRango > vHasta))
		{
			alert("Esta fuera del rango de fechas")
			document.Prueba.RangoFecha.value = ""
			calendario.show(document.Prueba.RangoFecha, "mmmm-dd-yyyy", calendario.formatDate(document.Prueba.DesdeFecha.value, "mmm.dd.yyyy", "mmmm-dd-yyyy"), calendario.formatDate(document.Prueba.HastaFecha.value,"yyyy/mmm/dd", "mmmm-dd-yyyy"))
		}
	}
}
...
function cerrar()
{
objPopCalendar.PopCalTimeOut=window.setTimeout("window.clearTimeout(objPopCalendar.PopCalTimeOut);objPopCalendar.PopCalTimeOut=null;PopCalHideCalendar()",100);
}